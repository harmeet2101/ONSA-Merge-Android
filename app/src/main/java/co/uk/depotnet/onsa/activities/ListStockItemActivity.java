...
public class ListStockItemActivity extends AppCompatActivity {
    ...
    private AdapterStockListItem adapter;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if(formItem.getListItemType().equalsIgnoreCase("mystores")){
            items = new ArrayList<>();

        }else {
            if (staId != null && !TextUtils.isEmpty(staId.getAnswer())) {
                items = DBHandler.getInstance().getStockItemsByStaId(staId.getAnswer());
            } else {
                items = DBHandler.getInstance().getStockItems();
            }
        }

        adapter = new AdapterStockListItem(this, items, selectedValues, true);
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String s) {
                ...
                s = s.toLowerCase();
                ArrayList<StockItems> keys = new ArrayList<>();

                for (StockItems itemType : items) {
